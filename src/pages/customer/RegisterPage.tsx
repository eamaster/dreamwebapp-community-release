...
export function RegisterPage() {
    const { register, capabilities, isAuthenticated } = useCustomerAuth();
            ...
            <div className="max-w-md w-full space-y-8 bg-slate-900/90 border border-slate-800 p-8 rounded-2xl shadow-2xl backdrop-blur-xl">
                <div>
                    ...
                    <p className="mt-2 text-center text-sm text-slate-400">
                        ...
                        <Link to={`/login?returnTo=${encodeURIComponent(returnTo)}`} className="font-medium text-brand-400 hover:text-brand-300 transition-colors">
                            Sign in
                        </Link>
                    </p>
                </div>
